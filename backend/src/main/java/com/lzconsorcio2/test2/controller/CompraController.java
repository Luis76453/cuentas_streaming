...
public class CompraController {
    ...
    // Crear compra con items (coincide con el frontend)
    /*@PostMapping("/purchases")
    @Transactional
    public ResponseEntity<Compra> registrarCompra(@RequestBody PurchaseRequestDTO request) {
        if (request.getUser_id() == null || request.getTotal() == null || 
        ...
        compra.setUser_id(request.getUser_id());
        compra.setTotal(request.getTotal());
        
        Compra nuevaCompra = compraService.registrarCompra(compra);
        
        // Crear los detalles de compra
        List<DetalleCompra> detalles = request.getItems().stream().map(item -> {
            DetalleCompra detalle = new DetalleCompra();
            detalle.setCompra_id(nuevaCompra.getId_compra());
            ...
            // El subtotal se calculará automáticamente en el service
            return detalle;
        }).collect(Collectors.toList());
        
        detalleCompraService.registrarDetalles(detalles);
        
        // Obtener la compra con sus detalles para la respuesta
        Optional<Compra> compraConDetalles = compraService.getCompraById(nuevaCompra.getId_compra());
        
        return ResponseEntity.ok(compraConDetalles.orElse(nuevaCompra));
    }*/
